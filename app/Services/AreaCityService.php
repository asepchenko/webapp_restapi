...
class AreaCityService extends BaseService
{
    protected $repo;

    public function __construct(
        AreaCityRepository $repo
    ) {
        parent::__construct();
        $this->repo = $repo;
    }
    ...
    {
        $data = $this->repo->getIndexByAreaID($id);
        if ($data != NULL) {
            return ResponseFormatter::success($data,'OK');
        } else {
            return ResponseFormatter::error($data,'Data Not Found','404');
    ...
    public function create(Request $request, array $data)
    {
        ...
        try {
            ...
            /*$validator = Validator::make($data, [
                'areacity_name' => 'required|max:255|unique:areacities,areacity_name'
            ]);
    
            if ($validator->fails()) {
                throw new Exception($validator->errors());
            }*/
            $alreadyExist = $this->repo->isAlreadyExist($request->area_id, $request->city_id);
            if ($alreadyExist) {
                throw new Exception('Tidak bisa menambah data yang sudah ada');
            }
    ...
    public function update(Request $request, array $data, $id)
    {
        ...
        try {
            //validating
            /*$validator = Validator::make($data, [
                'areacity_name' => ['required','max:255',Rule::unique('areacities', 'areacity_name')->ignore($id)]
            ]);
