...
class CustomerPicService extends BaseService
{
    protected $repo;

    public function __construct(
        CustomerPicRepository $repo
    ) {
        parent::__construct();
        $this->repo = $repo;
    }
    ...
    public function create(Request $request, array $data)
    {
        ...
        try {
            ...
            $validator = Validator::make($data, [
                ...
                'email' => 'required|email|max:191|unique:user_customers,email'
            ],
            [
    ...
    public function update(Request $request, array $data, $id)
    {
        ...
        try {
            $validator = Validator::make($data, [
                'name'  => 'required|string|max:191',
                'email' => ['required','email','max:191',Rule::unique('user_customers', 'email')->ignore($id)]
            ],
            [
                'name.required' => 'Nama tidak boleh kosong',
                'email.required'=> 'Email tidak boleh kosong', 
