...
class CustomerBrandService extends BaseService
{
    protected $repo;

    public function __construct(
        CustomerBrandRepository $repo
    ) {
        parent::__construct();
        $this->repo = $repo;
    }
    ...
    public function create(Request $request, array $data)
    {
        ...
        try {
            ...
            /*$validator = Validator::make($data, [
                'customerbrand_name' => 'required|max:255|unique:customerbrands,customerbrand_name'
            ]);
    ...
    public function update(Request $request, array $data, $id)
    {
        ...
        try {
            //validating
            /*$validator = Validator::make($data, [
                'customerbrand_name' => ['required','max:255',Rule::unique('customerbrands', 'customerbrand_name')->ignore($id)]
            ]);
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            if($check){
                throw new Exception('Master brand ini diguanakn pada master cabang, tidak bisa dihapus');
            }
