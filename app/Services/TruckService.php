...
class TruckService extends BaseService
{
    ...
    public function indexByTypeID($id)
    {
        $data = $this->repo->getIndexByTypeID($id);
        if ($data != NULL) {
            return ResponseFormatter::success($data,'OK');
        } else {
            return ResponseFormatter::error($data,'Data Not Found','404');
    ...
    public function create(Request $request, array $data)
    {
        ...
        try {
            ...
            /*$validator = Validator::make($data, [
                'truck_name' => 'required|max:255|unique:trucks,truck_name'
            ]);
    ...
    public function update(Request $request, array $data, $id)
    {
        ...
        try {
            //validating
            /*$validator = Validator::make($data, [
                'truck_name' => ['required','max:255',Rule::unique('trucks', 'truck_name')->ignore($id)]
            ]);
