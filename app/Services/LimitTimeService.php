...
use App\Repositories\LimitTimeRepository;

class LimitTimeService extends BaseService
{
    protected $repo;

    public function __construct(
        LimitTimeRepository $repo
    ) {
        parent::__construct();
        $this->repo = $repo;
    }
    ...
    public function create(Request $request, array $data)
    {
        ...
        try {
            ...
            /*$validator = Validator::make($data, [
                'limittime_name' => 'required|max:255|unique:limittimes,limittime_name'
            ]);
            ...
            $check = $this->repo->checkAlreadyExist($request->location_id);

            if($check){
                throw new Exception('Data ini sudah tersedia');
            }
    ...
    public function update(Request $request, array $data, $id)
    {
        ...
        try {
            //validating
            /*$validator = Validator::make($data, [
                'limittime_name' => ['required','max:255',Rule::unique('limittimes', 'limittime_name')->ignore($id)]
            ]);
